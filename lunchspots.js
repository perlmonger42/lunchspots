// Set up a collection to contain restaurant location information. On the 
// server, it is backed by a MongoDB collection named "venues".
  ...
  Meteor.startup(function () {
    if (Venues.find().count() === 0) {
      ...
      for (var i = 0; i < names.length; i++)
        Venues.insert({name: names[i], score: Math.floor(Random.fraction()*10)*5});
    }
  });
}
